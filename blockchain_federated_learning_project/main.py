...
class BlockchainFederatedIncentiveSystem:
        ...
        try:
            ...
            if self.config.use_tcn:
                ...
                self.model = TransductiveFewShotModel(
                    input_dim=self.config.input_dim,
                    hidden_dim=self.config.hidden_dim,  # Use all 57 features, let multi-scale extractors learn importance
                    embedding_dim=self.config.embedding_dim,
                    num_classes=2,   # Binary classification (Normal vs Attack)
    ...
    def evaluate_zero_day_detection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"FPR: {metrics.get('fpr', 0.0):.4f} | TPR: {metrics.get('tpr', 0.0):.4f}")
            logger.info(
    ...
    def evaluate_base_model_only(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Initialize zero-day mask defaults
            ...
            # Calculate TPR (True Positive Rate) = TP / (TP + FN) = Recall
            from sklearn.metrics import confusion_matrix
            base_cm_binary = confusion_matrix(y_true_bin, y_pred_bin)
            if base_cm_binary.shape == (2, 2):
                tn, fp, fn, tp = base_cm_binary.ravel()
                base_fpr = fp / (fp + tn) if (fp + tn) > 0 else 0.0
                base_tpr = tp / (tp + fn) if (tp + fn) > 0 else 0.0  # TPR = Recall
            else:
                base_fpr = 0.0
                base_tpr = 0.0
            ...
            base_results = {
                ...
                'f1_score': base_f1,  # Use macro F1-score for consistency with k-fold CV
                'f1_score_binary': base_f1_conventional,  # Keep binary F1 for reference
                'precision_macro': base_precision,
                'recall_macro': base_recall,
                'f1_score_macro': base_f1,
                'fpr': base_fpr,  # False Positive Rate
                'tpr': base_tpr,  # True Positive Rate (Recall)
            ...
            logger.info(f"      F1-Score (Macro): {base_f1:.4f} | F1-Score (Binary): {base_f1_conventional:.4f}")
            logger.info(f"      FPR: {base_fpr:.4f} | TPR: {base_tpr:.4f}")
    ...
    def perform_coordinator_side_ttt_adaptation(self) -> torch.nn.Module:
        ...
        try:
            ...
            # Perform TTT adaptation using coordinator's method (checks config.use_pseudo_labels)
            # Note: TTT is purely unsupervised - only query_x is used, no labels or support set
            # Use adapt_to_test_data which will choose the correct method based on config
            method = 'tent_pseudo' if self.config.use_pseudo_labels else 'tent'
            adapted_model = self.coordinator.adapt_to_test_data(
                query_x=query_x,
                query_y=None,  # No labels for unsupervised TTT
                config=self.config,
                method=method
            )
    ...
    def evaluate_adapted_model(self, adapted_model: torch.nn.Module) -> Dict[str, Any]:
        ...
        try:
            ...
            adapted_f1_binary = f1_score(y_test_binary, adapted_predictions_binary, zero_division=0)
            # Use macro F1-score for consistency with k-fold CV
            adapted_precision_macro, adapted_recall_macro, adapted_f1, _ = precision_recall_fscore_support(
                y_test_tensor.cpu().numpy(), adapted_predictions.cpu().numpy(), average='macro', zero_division=0
            )
            ...
            if adapted_cm_binary.shape == (2, 2):
                ...
                adapted_fpr = fp / (fp + tn) if (fp + tn) > 0 else 0.0
                adapted_tpr = tp / (tp + fn) if (tp + fn) > 0 else 0.0  # TPR = Recall
            else:
                adapted_fpr = 0.0
                adapted_tpr = 0.0
            ...
            adapted_results = {
                ...
                'f1_score': adapted_f1,  # Macro F1-score (consistent with k-fold CV)
                'f1_score_binary': adapted_f1_binary,  # Keep binary F1 for reference
                ...
                'fpr': adapted_fpr,  # False Positive Rate
                'tpr': adapted_tpr,  # True Positive Rate (Recall)
            ...
            logger.info(f"      F1-Score (Macro): {adapted_f1:.4f} | F1-Score (Binary): {adapted_f1_binary:.4f}")
            logger.info(f"      FPR: {adapted_fpr:.4f} | TPR: {adapted_tpr:.4f}")
    ...
    X_test: torch.Tensor,
     y_test: torch.Tensor) -> Dict:
        ...
        try:
            ...
            from sklearn.metrics import accuracy_score, f1_score, matthews_corrcoef, average_precision_score, confusion_matrix
            ...
            fold_f1_scores = []
            fold_mcc_scores = []
            fold_auc_pr_scores = []  # Track AUC-PR for each fold
            fold_fpr_scores = []  # Track FPR for each fold
            fold_tpr_scores = []  # Track TPR for each fold
    ...
    self,
     'coordinator') and self.coordinator and self.coordinator.model:
                ...
                with torch.no_grad():
                    ...
                    y_fold_binary = (y_fold.cpu().numpy() != 0).astype(int)
                    predictions_np = predictions.cpu().numpy()
                    attack_probs_np = attack_probabilities.cpu().numpy()
                    
                    # Calculate metrics
                    accuracy = accuracy_score(y_fold_binary, predictions_np)
                    f1 = f1_score(y_fold_binary, predictions_np, average='macro')
                    mcc = matthews_corrcoef(y_fold_binary, predictions_np)
                    
                    # Calculate FPR (False Positive Rate) = FP / (FP + TN)
                    # Calculate TPR (True Positive Rate) = TP / (TP + FN) = Recall
                    cm_binary = confusion_matrix(y_fold_binary, predictions_np)
                    if cm_binary.shape == (2, 2):
                        tn, fp, fn, tp = cm_binary.ravel()
                        fpr = fp / (fp + tn) if (fp + tn) > 0 else 0.0
                        tpr = tp / (tp + fn) if (tp + fn) > 0 else 0.0  # TPR = Recall
                    else:
                        fpr = 0.0
                        tpr = 0.0
                    
                    # Calculate AUC-PR (PRIMARY metric for imbalanced data)
                    try:
                        if len(np.unique(y_fold_binary)) > 1:  # Both classes present
                            auc_pr = average_precision_score(y_fold_binary, attack_probs_np)
                        else:
                            # Single class: use mean probability as proxy
                            auc_pr = np.mean(attack_probs_np) if y_fold_binary[0] == 1 else 1.0 - np.mean(attack_probs_np)
                    except Exception as e:
                        logger.warning(f"    ⚠️ AUC-PR calculation failed for fold {fold_idx + 1}: {e}")
                        auc_pr = 0.0

                    logger.info(
                        f"    📊 Fold {fold_idx + 1} metrics: Accuracy={accuracy:.4f}, F1={f1:.4f}, MCC={mcc:.4f}, AUC-PR={auc_pr:.4f}, FPR={fpr:.4f}, TPR={tpr:.4f}")
                    
                    fold_accuracies.append(accuracy)
                    fold_f1_scores.append(f1)
                    fold_mcc_scores.append(mcc)
                    fold_auc_pr_scores.append(auc_pr)
                    fold_fpr_scores.append(fpr)
                    fold_tpr_scores.append(tpr)
            ...
            results = {
                ...
                'auc_pr': np.mean(fold_auc_pr_scores),  # AUC-PR mean across folds
                'auc_pr_std': np.std(fold_auc_pr_scores),  # AUC-PR std across folds
                'fpr_mean': np.mean(fold_fpr_scores),  # FPR mean across folds
                'fpr_std': np.std(fold_fpr_scores),  # FPR std across folds
                'tpr_mean': np.mean(fold_tpr_scores),  # TPR mean across folds
                'tpr_std': np.std(fold_tpr_scores),  # TPR std across folds
    ...
    y_fold.cpu().numpy(),
     final_predictions.cpu().numpy())
            ...
            logger.info(f"  F1-Score (Macro): {results['macro_f1_mean']:.4f} ± {results['macro_f1_std']:.4f}")
            logger.info(f"  FPR: {results['fpr_mean']:.4f} ± {results['fpr_std']:.4f} | TPR: {results['tpr_mean']:.4f} ± {results['tpr_std']:.4f}")
            logger.info(f"  MCC: {results['mcc_mean']:.4f} ± {results['mcc_std']:.4f}")
            logger.info(f"  AUC-PR: {results['auc_pr']:.4f} ± {results['auc_pr_std']:.4f}")
    ...
    def _evaluate_ttt_model_kfold(
        ...
        try:
            from sklearn.model_selection import StratifiedKFold
            from sklearn.metrics import accuracy_score, f1_score, matthews_corrcoef, precision_score, recall_score, average_precision_score, confusion_matrix
            ...
            for fold_idx, (train_idx, val_idx) in enumerate(kfold.split(X_np, y_np)):
                ...
                try:
                    ...
                    with torch.no_grad():
                        ...
                        # Calculate FPR (False Positive Rate) = FP / (FP + TN)
                        # Calculate TPR (True Positive Rate) = TP / (TP + FN) = Recall
                        cm_binary = confusion_matrix(y_eval_binary, predictions_np)
                        if cm_binary.shape == (2, 2):
                            tn, fp, fn, tp = cm_binary.ravel()
                            fpr = fp / (fp + tn) if (fp + tn) > 0 else 0.0
                            tpr = tp / (tp + fn) if (tp + fn) > 0 else 0.0  # TPR = Recall
                        ...
                        attack_probs_np = attack_probabilities.cpu().numpy()
                        try:
                            if len(np.unique(y_eval_binary)) > 1:  # Both classes present
                                auc_pr = average_precision_score(y_eval_binary, attack_probs_np)
                            else:
                                # Single class: use mean probability as proxy
                                auc_pr = np.mean(attack_probs_np) if y_eval_binary[0] == 1 else 1.0 - np.mean(attack_probs_np)
                        except Exception as e:
                            logger.warning(f"    ⚠️ AUC-PR calculation failed for TTT fold {fold_idx + 1}: {e}")
                            auc_pr = 0.0
                        ...
                        logger.info(
                            ...
                            f"F1={f1:.4f}, MCC={mcc:.4f}, AUC-PR={auc_pr:.4f}, FPR={fpr:.4f}, TPR={tpr:.4f}")
                    ...
                    fold_auc_pr_scores.append(0.0)
                    fold_fpr_scores.append(0.0)
                    fold_tpr_scores.append(0.0)
...
def main():
    ...
    try:
        ...
        # Add TTT adaptation data (including gradient norms) if available
        if hasattr(system, 'ttt_adaptation_data') and system.ttt_adaptation_data:
            # Convert numpy arrays to lists for JSON serialization
            ttt_adaptation_for_json = {}
            for key, value in system.ttt_adaptation_data.items():
                if isinstance(value, (list, np.ndarray)):
                    # Convert numpy arrays to lists
                    if isinstance(value, np.ndarray):
                        ttt_adaptation_for_json[key] = value.tolist()
                    else:
                        # Handle lists that may contain numpy arrays
                        ttt_adaptation_for_json[key] = [
                            v.tolist() if isinstance(v, np.ndarray) else v 
                            for v in value
                        ]
                else:
                    ttt_adaptation_for_json[key] = value
            evaluation_results['ttt_adaptation'] = ttt_adaptation_for_json
            logger.info(f"✅ Added TTT adaptation data to evaluation_results (including gradient norms)")
